...
export async function update(id, amount) {
  ...
  try {
    ...
    log(
      `${displaySuccess()} ${displayText("Order")} `
      + `${displayID(id )} ${displayText("updated with amount")} ${displayAmount(amount)}`);
  } catch (err) {
...
export async function add(...args) {
  ...
  try {
    ...
    log(
      ...
      + `${displayID(id)} ${displayText("-")} `
      + `${displayName(name)} ${displayText("has been added to the")} `
